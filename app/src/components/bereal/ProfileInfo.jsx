...
const ProfileInfo = ({ user, data }) => {
    ...
    const getTime = () => {
        ...
        if (data.isLate)
        {
            ...
            if (late > 3600) lateString = Math.ceil(late / 3600) + " h in ritardo"    
            else if (late > 60) lateString = Math.ceil(late / 60) + " min in ritardo"    
            else lateString = late + " sec in ritardo"
        }

        var finalString = ""
        if (Object.keys(data).includes("origin")) if(data.origin == "repost") finalString += "🔃 " + data.parentPostUsername + " • "
        ...
        <>
            {
                user && data ?
                    <div className="profileInfo w-5/6 h-fit flex items-center gap-3">
                        {
                            user.profilePicture ?
                                <img src={user.profilePicture.url} alt="" className='w-20 rounded-full border-gray-500 border-2' />
                                :
